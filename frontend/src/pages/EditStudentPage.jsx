...
function EditStudentPage() {
  ...
  useEffect(() => {
    const fetchStudent = async () => {
      setPageLoading(true);
      try {
        // Assuming a GET /students/{student_id} endpoint exists
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = { ...studentData, semester: parseInt(studentData.semester, 10) };
      const response = await apiClient.put(`/students/${studentId}/`, payload);
      setSuccess(`Student "${response.data.name}" updated successfully!`);
      setTimeout(() => {
      ...
      setError(err.response?.data?.detail || err.message || 'Failed to update student.');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (pageLoading) return (
    <ResponsiveContainer className="text-center py-12">
      <div className="inline-flex items-center justify-center">
        ...
        <span className="font-playfair text-xl text-navy">Retrieving Student Information...</span>
      </div>
    </ResponsiveContainer>
  );
  
  if (error && !studentData.name) return (
    <ResponsiveContainer className="py-8 text-center">
      <div className="card-classic p-8">
        <div className="flex flex-col items-center">
          ...
          <h2 className="text-2xl font-playfair font-bold text-navy mb-4">Unable to Retrieve Student</h2>
    ...
    <ResponsiveContainer>
      ...
      <div className="relative mb-10">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-playfair font-bold text-navy mb-2">Edit Student</h1>
            <div className="h-1 w-24 bg-gold"></div>
            <p className="text-brown font-cormorant text-lg mt-2">{studentData.name || ''}</p>
          </div>
          <Link 
            to="/students" 
